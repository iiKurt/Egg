...
void render() {
    ...
    if (!knowsHowToPlay) {
        ...
        #if defined(__IPHONEOS__) || defined(__ANDROID__)
        PrintString(renderer, font, "Tap to confirm current character...", (SCREEN_WIDTH / 2) - ((35 * 8) / 2), (SCREEN_HEIGHT / 2) - 100);
        #else
        // 45 characters in help text, each one is 8 px long:
        PrintString(renderer, font, "Press [Space] to confirm current character...", (SCREEN_WIDTH / 2) - ((45 * 8) / 2), (SCREEN_HEIGHT / 2) - 100);
        #endif
    }
	...
	for (short i = 0; i < ArrayLength; i++) { // For each column
        ...
        // Underline currently selected character
		if (currentIndex == i) {
